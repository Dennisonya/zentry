...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Get analytics (full rows so the dashboard can compute month-over-month trends)
      ...
      // Owner display info for the greeting/avatar
      const { data: profile } = await supabase
        .from("profiles")
        .select("full_name, avatar_url")
        .eq("id", user.id)
        .maybeSingle()
      setOwnerName((profile as { full_name?: string } | null)?.full_name || null)
