...
export function ClassicCardLayout({ business, products }: ClassicCardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="border-b" style={{ backgroundColor: accentColor, color: "#ffffff" }}>
            ...
            {products.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                  Click below to send an inquiry or book a service with us
                </p>
                <Button size="lg" onClick={() => setServiceInquiryOpen(true)} style={{ backgroundColor: accentColor }}>
                    ...
                    <Card key={product.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                      {product.image_url && (
                        <div className="aspect-square overflow-hidden bg-muted">
                          <img src={product.image_url || "/placeholder.svg"} alt={product.name} className="h-full w-full object-cover hover:scale-105 transition-transform" />
                        </div>
                      )}
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Button size="sm" onClick={() => setServiceInquiryOpen(true)} style={{ backgroundColor: accentColor }}>
                    ...
                    <Card key={product.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-lg font-bold">${product.price.toFixed(2)}</span>
                          {business.whatsapp_number ? (
                            <Button size="sm" onClick={() => handleWhatsAppOrder(product)}>
                              Order Now
                            </Button>
                          ) : (
